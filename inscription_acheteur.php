...
// if(!($Mail==""||$Mdp==""||$Nom==""||$Prenom==""||$Adresse==""||$Ville==""||$CodePostal==""||$Pays==""||$Tel==""||$NumCarte==""||$DateCarte==""||$NomCarte==""||$Crypto==""||$Civilite==""||$DateNaissance==""))
// {
  $database = "Projet";
  ...
  if($db_found)
  {
    ...
    $sql = 'INSERT INTO Acheteur(Mail, Mdp, Nom, Prenom, Adresse, Ville, CodePostal, Pays, Tel, NumCarte, DateCarte, NomCarte, Crypto, Civilite, DateNaissance) VALUES("'.$Mail.'", "'.$Mdp.'", "'.$Nom.'", "'.$Prenom.'", "'.$Adresse.'", "'.$Ville.'", "'.$CodePostal.'", "'.$Pays.'", "'.$Tel.'", "'.$NumCarte.'", "'.$DateCarte.'", "'.$NomCarte.'","'.$Crypto.'", "'.$Civilite.'", "'.$DateNaissance.'")';
    mysqli_query($db_handle, $sql);
